<?php
/**
 * Plugin Name: Arianna: Classic Blog Module
 ...
 */
add_action('widgets_init', 'arianna_register_classic_blog_module');

function arianna_register_classic_blog_module(){
	register_widget('arianna_classic_blog');
}
 ...
 *
 */ 
class arianna_classic_blog extends WP_Widget {
	...
	function __construct(){
		/* Widget settings. */	
		$widget_ops = array('classname' => 'module-classic-blog', 'description' => esc_html__('[Full-width module][Content module] Displays latest posts in classic blog layout in content section.','arianna'));
		
		/* Create the widget. */
		parent::__construct('arianna_classic_blog', esc_html__('*arianna: Module Classic Blog','arianna'), $widget_ops);
	}
	...
	function widget($args, $instance){	
        ...
        $classic_layout = 'layout1';
        if($layout == 'layout1') {
            $classic_layout = 'layout1';
        }elseif($layout == 'layout2') {
            $classic_layout = 'layout2';
        }else {
            $classic_layout = 'layout1';
        }
        $query = new WP_Query( $args ); ?>
        <div id="<?php echo esc_attr($uid);?>" class="arianna_classic-blog-wrapper <?php echo esc_attr($classic_layout);?>" >
         	
    		<div class="classic-blog-content-container">
                <?php while ( $query -> have_posts() ) : $query -> the_post(); ?>
                    <?php $post_id = get_the_ID();?>
                    <?php echo arianna_classic_blog_render($post_id, $excerpt_length);?>
                <?php endwhile;?>
            </div>	
	...
	function form($instance){
		...
		$defaults = array('title' => '', 'category' => 'all', 'layout' => 'layout1', 'entries_display' => 6, 'entries_loadmore' => 4, 'post_offset' => 0, 'excerpt_length' => 25);
        ...
        <p>
			<label for="<?php echo $this->get_field_id('layout'); ?>"><strong><?php esc_attr_e('Layout:', 'arianna'); ?></strong></label> 
			<select id="<?php echo $this->get_field_id('layout'); ?>" name="<?php echo $this->get_field_name('layout'); ?>" class="widefat widget-option" >
				<option value='layout1' <?php if ($instance['layout'] == 'layout1') echo 'selected="selected"'; ?>><?php esc_attr_e( 'Layout 1', 'arianna' ); ?></option>
                <option value='layout2' <?php if ($instance['layout'] == 'layout2') echo 'selected="selected"'; ?>><?php esc_attr_e( 'Layout 2', 'arianna' ); ?></option>
			</select>
		</p>
