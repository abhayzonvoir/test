...
class Paypal extends Payment
{
	...
	public static function sendbannerPayment($post)
	{
		 
		// Set URLs
		parent::$uri['previousUrl'] =  url('/').'/bottombanner/'.$post['post_id'].'/payment';
		parent::$uri['nextUrl'] = url('/').'/bottombanner/'.$post['post_id'].'/payment';
		parent::$uri['paymentCancelUrl'] = url('/').'/bottombanner/'.$post['post_id'].'/cancel';
		parent::$uri['paymentReturnUrl'] =url('/').'/bottombanner/'.$post['post_id'].'/success';
		
		// Get the Package
		$package = Bannerpackages::find($post['package_id']);
	...
	public static function sendPaymentbanner($params, $post)
	{
		...
		try {
			...
			if (isset($rawData['PAYMENTINFO_0_ACK']) && $rawData['PAYMENTINFO_0_ACK'] === 'Success') {
				...
				if (isset($rawData['PAYMENTINFO_0_TRANSACTIONID'])) {
				 ...
				 return redirect('/bottombanner/successbanner?transaction_id='.$params['transaction_id'])->with('transaction_id', $params['transaction_id']);
				// Apply actions after successful Payment
